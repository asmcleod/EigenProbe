...
def GetQuadrature(N=72,xmin=1e-3,xmax=numpy.inf,\
    ...
    if hasattr(quadrature,'calc_nodes'):
        ...
        else: raise NotImplementedError('Quadrature method %s is not implemented!'%quadrature)
...
def ParameterFit(xs,ys,model_func,params0,
                 limits=None,relative_error=False,weights = None,
                 error_exp=2,limit_sharpness=10,
                 args=(),**kwargs):
    ...
    #                      'greater or equal in length to the number of fit parameters.',\
    ...
    if relative_error:
        Diff=lambda model,ys: np.abs((model-ys)/ys)**error_exp * weights
    else: Diff=lambda model,ys: np.abs(model-ys)**error_exp * weights
        
    #Define error function#
    def error_func(params,*args,dy=1):
        
        model=model_func(xs,params,*args)
        window=window_func(params)
        
        return Diff(model,ys)*window*1/dy
...
def PiecewiseLinearFit(x,y,nbreakpoints=1,**kwargs):
    ...
    dx = Dx/(nbreakpoints+1)
    y0=np.mean(y)

    params = x0 + dx*(1+np.arange(nbreakpoints))
    ...
    return model_func(x,params),params
